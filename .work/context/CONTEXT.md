...
Product / UX source of truth: **`.work/plans/legacy-plans/proposal/preliminary.md`**.  
**Prioritized implementation backlog:** **`.work/plans/NEXT.md`**. Full MVP spec: **`.work/plans/legacy-plans/proposal/20260515-full-project.md`**.  
Rolling session snapshot: **`.work/context/HANDOFF.md`**.
...
- Persisted: **`users`**, **`projects`**, **`project_members`**, **`components`**, **`tasks`**, **`activities`**, **`mentions`**, **`tickets`**, **`attachments`** (ticket and/or **task** / project-scoped rows), **`inbox_items`**, **`watchers`**, **`github_links`**, **`github_commits`**. Schema in **`sql/schema_*.sql`** aligned with **`api/app/models`**.
- Web UX: Kanban + task detail, command palette (**⌘K**), project health pills, **Inbox**, **watch** actions — see **`NEXT.md`** implementation status.
- **GitHub (Batch I, partial):** REST **`/v1/projects/{id}/github/*`** + background poll + **`html_url`** on cached commits — see **`NEXT.md` § I12** (no dedicated Next.js GitHub page yet).
- Optional polish: richer notifications; attachment retention purge; **`github_commit`** activity feed — see **`NEXT.md`** follow-ups.
