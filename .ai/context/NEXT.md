...
**Latest (repo):** **2026-05-16** — Phase **1** (**Batch G**) and Phase **2** (**Batch H** + **P2–P5**) **complete** on `main`: **`GET /v1/me/refs/search`** + web BFF **`/api/me/refs/search`**, per-project **byte** cap (`attachment_max_bytes_per_project` / **`ATTACHMENT_MAX_BYTES_PER_PROJECT`**), Markdown **`@`** + **`#`** on **project Activity** (composer + replies) and **ticket discussion**. **Deferred:** scheduled **retention purge** (only `retention_cutoff()` hook + env today). **`sql/`**: **`inbox_items`**, **`watchers`**, **`attachments`** with nullable **`task_id`** / **`ticket_id`**. Batch **A** whenever models change.
...
────────────────────────────────────────────────
Matrix rows     ████████████████████  14/14 Done   (0 partial)

Open follow-up: retention cron · optional Inbox "c" shortcut · Batch I (GitHub)
```
...
| **H4** | Markdown-ish editor | **Done** | **`MarkdownEditor`** on **project** composer + threaded replies and **ticket** discussion with **`mentionSuggestions`** (`GET /v1/me/users/search` + BFF) and **`refSuggestions`** (`GET /v1/me/refs/search` + `web/src/app/api/me/refs/search/route.ts`). |
...
| **H** | Inbox, watches, SSE payload, task activity, markdown editor | **Done** (see Implementation status) |
