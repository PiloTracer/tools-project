#!/usr/bin/env bash
# UI Design OS release preflight - a tag cannot ship while verification is red.
#
# Usage: bash scripts/release.sh <version>        # e.g. 0.5.0   (no leading 'v')
#
...
if [[ $# -lt 1 ]]; then
  echo "usage: bash scripts/release.sh <version>   (e.g. 0.5.0)" >&2
  exit 2
fi
version="${1#v}"
tag="v${version}"
...
for v in framework-verify readiness-verify traceability-verify; do
