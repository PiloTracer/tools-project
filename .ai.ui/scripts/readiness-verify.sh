#!/usr/bin/env bash
# UI Design OS readiness-verify - mechanically enforce PROBE_LEDGER honesty rules.
#
# Validates each probe ledger (default: all .work.ui/**/PROBE_LEDGER.md under cwd)
# against the rules in .ai.ui/skills/probe-protocol.md:
#   1. Honesty: a dimension marked confirmed/high MUST cite evidence (no empty / dash / TBD).
#   2. Coverage math: header "Coverage: NN%" must match the value recomputed from the
...
#
# Usage:
#   bash readiness-verify.sh                  # scan .work.ui/ for PROBE_LEDGER.md
...
if [[ $# -gt 0 ]]; then
  ...
  while IFS= read -r -d '' f; do ledgers+=("$f"); done \
    < <(find .work.ui -name 'PROBE_LEDGER.md' -print0 2>/dev/null || true)
fi
...
note "UI Design OS readiness-verify (${#ledgers[@]} ledger(s))"
...
for ledger in "${ledgers[@]}"; do
  ...
  result="$(awk '
    ...
    # Data rows: 2nd cell starts with a UI dimension id (D1, B3, ...)
    {
      dim=trim($2)
      if (dim !~ /^(D|B)[0-9]+/) next
      rows++
